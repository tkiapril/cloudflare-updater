...
for account in update_list:
    ...
    for target in account['targets']:
        ...
        if target['subdomain'].count('.') >= 2:
            zone = zone[zone.rfind('.', 0, zone.rfind('.'))+1:]
        ...
        p.rec_edit(
            ...
            target_record['name'],
            myip,
            ttl=int(target_record['ttl']),
            service_mode=int(target_record['service_mode'])
        )
        print(
            'Successfully updated {} to {}'.format(target['subdomain'], myip)
        )
